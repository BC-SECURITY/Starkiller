<template>
  <div>
    ...
    >
      ...
      <template v-if="!plugin" #item.plugin_id="{ item }">
        <router-link
          style="color: inherit"
          :to="{ name: 'pluginEdit', params: { id: item.plugin_id } }"
        >
          {{ item.plugin_id }}
        </router-link>
      </template>
...
export default {
  ...
  methods: {
    submitHeaderForm(val) {
      ...
      this.applicationStore.pluginTaskHeaders = [...this.selectedHeadersTemp];
    },
    resetHeaders() {
      this.applicationStore.pluginTaskHeaders = this.headersFull.filter(
        (h) => h.defaultHeader === true,
    ...
    updateTag(task, tag) {
      pluginApi
        .updateTag(task.plugin_id, task.id, tag)
        .then((t) => {
